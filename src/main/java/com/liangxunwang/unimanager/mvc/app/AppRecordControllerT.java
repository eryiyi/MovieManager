...
@Controller
public class AppRecordControllerT extends ControllerConstants {

    @Autowired
    @Qualifier("appRecordServiceT")
...
//    private DeleteService deleteRecordService;
    ...
    //首页获得求购供应信息
    @RequestMapping(value = "/recordListT", produces = "text/plain;charset=UTF-8")
    @ResponseBody
    public String recordListT(RecordQuery query, Page page){
    ...
    //个人主页获得求购供应信息
    @RequestMapping(value = "/recordListByIdT", produces = "text/plain;charset=UTF-8")
    @ResponseBody
    public String recordListByIdT(RecordQuery query, Page page){
        query.setIndex(page.getIndex()==0?1:page.getIndex());
        query.setSize(query.getSize()==0?page.getDefaultSize():query.getSize());
        try {
            Object[] results = (Object[]) recordListService.list(query);
    ...
    @RequestMapping(value = "/sendRecordT", produces = "text/plain;charset=UTF-8;")
    @ResponseBody
    public String save(RecordT record, HttpSession session){
        try {
    ...
    @RequestMapping(value = "/getRecordByIdT", produces = "text/plain;charset=UTF-8;")
    @ResponseBody
    public String getRecordById(String id,String accessToken){
        try {
            RecordVOT record = (RecordVOT) findRecordService.findById(id);
            DataTip tip = new DataTip();
            tip.setData(record);
            return toJSONString(tip);
        }catch (ServiceException e){
